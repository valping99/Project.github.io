...
public class ShoppingDA {
    ...
    public List<SanPham> getProPage1(Integer offset, Integer maxResult) {
        Query q = s.createQuery("from SanPham where trangThai like '1'");
        return q.setFirstResult(offset == null ? 0 : offset).setMaxResults(maxResult == null ? 8 : maxResult).list();
    }
    ...
    public List<SanPham> getSanPhamByQuan() {
        Query q = s.createQuery("from SanPham where maNhomSp like '%3%'");
        return q.list();
    }
    
    public List<SanPham> getSanPhamByAo() {
        Query q = s.createQuery("from SanPham where maNhomSp like '%1%'");
        return q.list();
    }
    
    public List<SanPham> getSanPhamByGiay() {
        Query q = s.createQuery("from SanPham where maNhomSp like '%2%'");
        return q.list();
    }
